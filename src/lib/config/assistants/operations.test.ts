...
test("enableAssistant writes to project config by default in project context", async () => {
  ...
  const projectConfig = await readJson<{ assistants: Record<string, string> }>(join(projectDir, ".skiui", "skiui.json"));
...
test("enableAssistant creates local config when targeting local scope", async () => {
  ...
  const localConfig = await readJson<{ assistants: Record<string, string> }>(join(projectDir, ".skiui", "skiui.local.json"));
...
test("disableAssistant writes to global config outside project context", async () => {
  ...
  const globalConfig = await readJson<{ assistants: Record<string, string> }>(join(globalDir, "skiui.json"));
