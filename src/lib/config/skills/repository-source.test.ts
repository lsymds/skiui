...
test("inferRepositoryName derives names from normalized sources", () => {
	...
	expect(inferRepositoryName({ type: "fs", path: ".skiui/local" })).toBe(
		"local",
	)
})
...
test("allocateRepositoryName appends numeric suffixes", () => {
	const repositories = [
		{
			name: "agent-skills",
			source: { type: "git", url: "x" } as const,
			skills: [],
		},
		{
			name: "agent-skills-2",
			source: { type: "git", url: "y" } as const,
			skills: [],
		},
	]

	expect(allocateRepositoryName("agent-skills", repositories)).toBe(
		"agent-skills-3",
	)
})
