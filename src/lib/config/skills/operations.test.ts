...
test("addSkill writes to project config by default when project config exists", async () => {
	...
	const projectConfigContents = await readFile(
	...
	const repository = projectConfig.repositories.find(
		(candidate) =>
			candidate.source.type === "git" &&
			candidate.source.url === VERCEL_AGENT_SKILLS_REPOSITORY,
	)
	...
	expect(
		repository?.skills.some(
			(skill) => skill.name === "my-skill" && skill.enabled,
		),
	).toBe(true)
...
test("addSkill falls back to global config outside project context", async () => {
	...
	const globalConfigContents = await readFile(
...
test("addSkill does not duplicate repository when git URL formatting differs", async () => {
	...
	const projectConfig = JSON.parse(
		await readFile(join(projectDir, ".skiui", "skiui.json"), "utf8"),
	) as {
		repositories: Array<{
			source: { type: string; url?: string }
	...
	expect(agentSkillRepos[0]?.skills.map((skill) => skill.path).sort()).toEqual([
		"first-skill",
		"second-skill",
	])
})
...
test("addSkill does not duplicate repository when fs path formatting differs", async () => {
	...
	const localRepos = projectConfig.repositories.filter(
		(repository) =>
			...
			repository.source.path === ".skiui/local",
	)

	expect(localRepos).toHaveLength(1)
	expect(localRepos[0]?.skills.map((skill) => skill.path).sort()).toEqual([
		"first-fs-skill",
		"second-fs-skill",
	])
})
...
test("listEnabledSkills includes scope for enabled entries", async () => {
	...
	expect(
		result.entries.some(
			(entry) => entry.scope === "global" && entry.skillName === "global-skill",
		),
	).toBe(true)
	expect(
		result.entries.some(
			(entry) =>
				entry.scope === "project" && entry.skillName === "project-skill",
		),
	).toBe(true)
	expect(
		result.entries.some(
			(entry) => entry.scope === "local" && entry.skillName === "local-skill",
		),
	).toBe(true)
})
