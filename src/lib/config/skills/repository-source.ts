...
export function isSameSource(
	...
	source: RepositoryConfig["source"],
): boolean {
...
export function inferRepositoryName(
	...
	if (source.type === "git") {
		...
		const colonSplit = sourceText.includes(":")
			? sourceText.split(":")
			: [sourceText]
		const finalSegment = colonSplit[colonSplit.length - 1] ?? sourceText
		const slashSegments = finalSegment
			.split("/")
...
export function normalizeFsPath(path: string): string {
	...
	const normalized = normalize(
		withoutTrailingSlash.length > 0 ? withoutTrailingSlash : trimmed,
	)
...
export function allocateRepositoryName(
	baseName: string,
