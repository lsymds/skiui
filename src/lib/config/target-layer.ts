...
export function selectTargetLayer(layers: LoadedLayers, scope: ConfigScope | undefined): TargetLayer {
  if (scope === "global") {
    if (!layers.global.config) {
      throw new CliError("No global skiui configuration found. Run `skiui init --scope global` first.");
    }
