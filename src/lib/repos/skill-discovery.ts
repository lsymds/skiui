...
export async function discoverSkills(
	...
	for (const skillFile of skillFiles) {
		...
		const relativeDirectory = toConfigPath(
			relative(skillRootPath, skillDirectory),
		)
...
async function parseSkillMetadata(
	...
	const name =
		headingMatch?.[1]?.trim() || frontmatter?.metadata.name || fallbackName

	const description =
		frontmatter?.metadata.description ?? findFirstDescriptionLine(body)
