...
test("cli apply links enabled project skills and reports missing skills", async () => {
	...
	const addMissingSkillResult = await runCli(
		["skill", "enable", "local", "missing-skill"],
		{ cwd: projectDir, env },
	)
	expect(addMissingSkillResult.exitCode).toBe(0)

	const applyWithMissingResult = await runCli(["apply"], {
		cwd: projectDir,
		env,
	})
	expect(applyWithMissingResult.exitCode).toBe(1)
	expect(applyWithMissingResult.stderr).toContain(
		"Enabled skills were not found in repository sources",
	)
