...
test("cli add-repo allows explicit repository name", async () => {
  ...
  await writeFile(join(sourceRoot, "my-skill", "SKILL.md"), "# My Skill\n", "utf8");

  const addRepoResult = await runCli(["add-repo", ".skiui/external-skills", "--name", "external"], {
    cwd: projectDir,
    env
  });
  ...
  expect(await fileExists(join(projectDir, ".skiui", "repos", "external", "my-skill", "SKILL.md"))).toBe(true);

  const addSkillResult = await runCli(["enable-skill", "external", "my-skill"], { cwd: projectDir, env });
  ...
  expect(
    parsed.repositories.some(
      (repository) =>
        ...
        repository.skills.some((skill) => skill.path === "my-skill" && skill.enabled)
    )
  ).toBe(true);
});
...
test("cli add-repo rejects invalid explicit repository name", async () => {
  ...
  const addRepoResult = await runCli(["add-repo", ".skiui/local", "--name", "Invalid Name"], { cwd: projectDir, env });
  expect(addRepoResult.exitCode).toBe(1);
  expect(addRepoResult.stderr).toContain("Repository name must contain only lowercase letters");
});
