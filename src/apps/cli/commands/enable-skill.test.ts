...
test("cli enable-skill errors when repository name does not exist", async () => {
  ...
  const result = await runCli(["enable-skill", "missing", "my-skill"], { cwd: projectDir, env });
  expect(result.exitCode).toBe(1);
  expect(result.stderr).toContain("Repository `missing` was not found in project config");
});
