				...
				.command<AgentArgs>(
					...
					async (args) => {
						...
						if (!result.statusChanged) {
							console.log(
								`Assistant \`${result.assistantId}\` already enabled in ${result.scope} config`,
							)
							return
						}

						console.log(
							`Updated ${result.scope} config: enabled assistant \`${result.assistantId}\``,
						)
					},
				)
				.command<AgentArgs>(
					...
					async (args) => {
						...
						if (!result.statusChanged) {
							console.log(
								`Assistant \`${result.assistantId}\` already disabled in ${result.scope} config`,
							)
							return
						}

						console.log(
							`Updated ${result.scope} config: disabled assistant \`${result.assistantId}\``,
						)
					},
				)
