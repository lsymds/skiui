...
test("cli agent enable rejects unsupported assistant id", async () => {
  ...
  const result = await runCli(["agent", "enable", "unknown-assistant"], { cwd: projectDir, env });
...
test("cli agent enable can write to local scope", async () => {
  ...
  const enableResult = await runCli(["agent", "enable", "claude", "--scope", "local"], { cwd: projectDir, env });
